...
const int PRED = 70.0;
void DoubleFilterORC(std::ostream &out, const char *filename, uint64_t batchSize,
...
//        printer->reset(*batch);
        ...
        orc::DoubleVectorBatch* dvector = dynamic_cast<orc::DoubleVectorBatch*>(structBatch->fields[0]);
        batches += 1;
        rows += batch->numElements;
        for (uint64_t r = 0; r < batch->numElements; ++r) {
            value = dvector->data[r];
            if (batch->notNull[r]){
                if (value > PRED) {
                    qualified++;
                }
    ...
    std::cout << "number of double value grater than " << PRED << ": " << qualified << std::endl;
}
...
//                ofs << "3" << std::endl;
                ...
                DoubleFilterORC(std::cout, argv[i], batchSize, rowReaderOptions);
