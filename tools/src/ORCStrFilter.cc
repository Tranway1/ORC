...
const std::string PRED = "Primary";
void scanStr(std::ostream &out, const char *filename, uint64_t batchSize,
...
//        printer->reset(*batch);
        ...
        orc::StringVectorBatch* strVector = dynamic_cast<orc::StringVectorBatch*>(structBatch->fields[0]);
        batches += 1;
        rows += batch->numElements;
        for (uint64_t r = 0; r < batch->numElements; ++r) {
            char value[strVector->length[r]+1];
            memcpy(value,strVector->data[r],static_cast<size_t>(strVector->length[r]));
            value[strVector->length[r]]='\0';
            if (batch->notNull[r]){
//                std::cout <<strVector->length[r]<< "cur: " << value << std::endl;
                if (value == PRED) {
                    qualified++;
                }
            }
...
//                ofs << "3" << std::endl;
                ...
                scanStr(std::cout, argv[i], batchSize, rowReaderOptions);
